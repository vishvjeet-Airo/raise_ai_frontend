...
const CardHeader = ({ title, icon, onAdd }: { title: string; icon: React.ReactElement; onAdd?: () => void; }) => (
    <div className="flex justify-between items-center mb-4 border-b pb-2">
        ...
        {onAdd && (
            <button onClick={onAdd} className="flex items-center gap-2 text-sm text-[#1F4A75] font-semibold p-2 rounded-lg hover:bg-gray-100">
                <Plus size={16} />Add
            </button>
        )}
    </div>
...
const ListItem = ({ modelType, item, children, onEdit, onDelete }: { modelType: ModelType; item: { id: number }; children: React.ReactNode; onEdit: (modelType: ModelType, data: any) => void; onDelete: (modelType: ModelType, id: number) => void; }) => (
    <div className="flex justify-between items-center p-2 rounded-md hover:bg-gray-50">
        <div>{children}</div>
        <div className="flex gap-3 flex-shrink-0 ml-4">
            <button onClick={() => onEdit(modelType, item)} className="text-gray-500 hover:text-blue-600"><Pencil size={16} /></button>
            <button onClick={() => onDelete(modelType, item.id)} className="text-gray-500 hover:text-red-600"><Trash2 size={16} /></button>
        </div>
    </div>
);

const EditModal = ({ modalConfig, onClose, onSave, orgId }: { modalConfig: { mode: 'create' | 'edit'; modelType: ModelType; data?: any; }, onClose: () => void, onSave: (modelType: ModelType, itemData: any, orgId: number) => void, orgId: number }) => {
...
export default function CompanyProfileByIdPage() {
    ...
    return (
        <>
        {modalConfig && profile && (
            <EditModal modalConfig={modalConfig} onClose={() => setModalConfig(null)} onSave={handleSave} orgId={profile.id} />
        )}
            ...
            <div className="flex-1 p-8 h-screen overflow-y-auto">
                ...
                <div className="flex flex-col gap-8">
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Jurisdictions" icon={<Globe />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'jurisdiction' })} />
                        {profile.jurisdictions?.length ? profile.jurisdictions.map(j => (
                            <ListItem key={j.id} modelType="jurisdiction" item={j} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <span>{j.city}, {j.state}, {j.country}</span>
                            </ListItem>
                        )) : <p className="text-gray-400">No jurisdictions added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Business Units" icon={<Briefcase />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'business_unit' })} />
                        {profile.business_units?.length ? profile.business_units.map(bu => (
                            <ListItem key={bu.id} modelType="business_unit" item={bu} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{bu.name}</p><p className="text-xs text-gray-500">Head: {bu.head_name || 'N/A'}</p></>
                            </ListItem>
                        )) : <p className="text-gray-400">No business units added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Licenses & Registrations" icon={<FileText />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'license' })} />
                        {profile.licenses?.length ? profile.licenses.map(lic => (
                            <ListItem key={lic.id} modelType="license" item={lic} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{lic.license_name}</p><p className="text-xs text-gray-500">Expires: {lic.expiry_date || 'N/A'}</p></>
                            </ListItem>
                        )) : <p className="text-gray-400">No licenses added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Regulators" icon={<Landmark />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'regulator' })} />
                        {profile.regulators?.length ? profile.regulators.map(reg => (
                            <ListItem key={reg.id} modelType="regulator" item={reg} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <span>{reg.name} ({reg.jurisdiction})</span>
                            </ListItem>
                        )) : <p className="text-gray-400">No regulators added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Standards Adopted" icon={<ClipboardCheck />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'standard' })} />
                        {profile.standards?.length ? profile.standards.map(std => (
                            <ListItem key={std.id} modelType="standard" item={std} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{std.name}</p><p className="text-xs text-gray-500">Valid Until: {std.valid_until || 'N/A'}</p></>
                            </ListItem>
                        )) : <p className="text-gray-400">No standards added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Critical Processes" icon={<AlertTriangle />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'critical_process' })} />
                        {profile.critical_processes?.length ? profile.critical_processes.map(cp => (
                            <ListItem key={cp.id} modelType="critical_process" item={cp} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{cp.name}</p><p className="text-xs text-gray-500">{cp.description}</p></>
                            </ListItem>
                        )) : <p className="text-gray-400">No critical processes added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Third-Party Dependencies" icon={<Users />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'third_party' })} />
                        {profile.third_parties?.length ? profile.third_parties.map(tp => (
                            <ListItem key={tp.id} modelType="third_party" item={tp} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{tp.name}</p><p className="text-xs text-gray-500">{tp.service_provided}</p></>
                            </ListItem>
                        )) : <p className="text-gray-400">No third-party dependencies added.</p>}
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <CardHeader title="Compliance History" icon={<History />} onAdd={() => setModalConfig({ mode: 'create', modelType: 'compliance_record' })} />
                        {profile.compliance_history?.length ? profile.compliance_history.map(ch => (
                            <ListItem key={ch.id} modelType="compliance_record" item={ch} onEdit={(mt, data) => setModalConfig({ mode: 'edit', modelType: mt, data })} onDelete={handleDelete}>
                                <><p className="font-semibold">{ch.record_type} ({ch.date})</p><p className="text-xs text-gray-500">{ch.description} - <strong>{ch.outcome}</strong></p></>
                            </ListItem>
